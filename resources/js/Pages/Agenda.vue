...
import Appointment from '@/components/AppointmentForm.vue';
...
const fetchTimes = async () => {
     try {
          const response = await fetch(`http://localhost:3000/api/consults/free-times?date=${form.date}`);
          if (!response.ok) throw new Error('Erro ao buscar os horários disponíveis');
     ...
     <main class="conteudo container" id="home">
          ...
          >
               <video
                    ...
                    width="1280"
                    height="720"
                    src="/public/6113143-hd_1280_720_60fps.mp4"
               ></video>
               <img
                    v-else
                    src="/public/bg-barber.jpg"
                    class="absolute inset-0 object-cover -z-10 size-full shadow-sm shadow-slate-700 m-auto mt-6 rounded-lg max-smallscreen:w-full max-smallscreen:h-44"
                    alt=""
               />
